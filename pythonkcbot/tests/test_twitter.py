...
def test_create_api(mock_api, mock_oauthhandler, mock_getenv):
    with patch("chalicelib.twitter.logger") as mock_logger:
        ...
        mock_oauthhandler.assert_called_with(
            os.getenv("CONSUMER_KEY"), os.getenv("CONSUMER_SECRET")
        )
        mock_oauthhandler.return_value.set_access_token.assert_called_with(
            os.getenv("ACCESS_TOKE"), os.getenv("ACCESS_TOKEN_SECRET")
        )
        mock_api.assert_called_with(
            mock_oauthhandler.return_value,
